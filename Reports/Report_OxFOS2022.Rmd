...
Awareness and usage varied a lot between practices (`r overall_unawareness[4]`% unaware respondents on average, range: `r overall_unawareness[1]`% -`r overall_unawareness[6]`%; `r overall_practice[4]`% practicing respondents, range: `r overall_practice[1]`% - `r overall_practice[6]`% ) and to some extend varied per Division. A large proportion of respondents was aware of open research practices but did not practice them (`r overall_aware_unpracticing_av[4]`% of respondents on average, range: `r overall_aware_unpracticing_av[1]`% -`r overall_aware_unpracticing_av[6]`%), even when excluding students who may have had less opportunity to practice them (`r researchers_aware_unpracticing_av[4]`% of respondents on average, range: `r researchers_aware_unpracticing_av[1]`% -`r researchers_aware_unpracticing_av[6]`%). Is this due to downsides or barriers? 

The widespread adoption of ORPs was largely presumed to be beneficial (`r overall_beneficial[4]`% of respondents on average, range: `r overall_beneficial[1]`% -`r overall_beneficial[6]`%, variation matching the inverse variation in rates of 'unsure' replies), and the majority of respondents (`r overall_no_downsides[4]`% on average, `r overall_no_downsides[1]`% -`r overall_no_downsides[6]`%, variation also corresponding to rates of 'unsure' replies) thought they were no downside associated with each ORP (with notable exceptions).

Around `r overall_atleastone_barrier[4]`% of respondents thought there were at least one barrier to adoptions of ORPs, especially a lack of training, norms and incentives, followed by a lack of infrastructure, and policy. Another `r overall_notsureNA_barrier[4]`% specified they weren't sure or thought the question wasn't applicable to them. Finally, `r overall_nobarrier[4]`% on average thought there were no barriers to ORPs.


There was a huge demand in training (online guidance and workshops, `r overall_training_need_av[4]`% on average, range: `r overall_training_need_av[1]`% - `r overall_training_need_av[6]`%), and in community support (e.g. mentorship, support networks; `r overall_support_need_av[4]`% on average, range: `r overall_support_need_av[1]`% - `r overall_support_need_av[6]`%).

There was a general agreement on the direction of change between currently used vs desirable recruitment criteria that actually aligns with the DORA principles (no JIF based assessment); and there were variations in the importance of specific criteria between Divisions. 


### Recommendations

To raise awareness and adoption, the first recommendation would be to provide a comprehensive and systematic training to all staff and students, with a modular structure to account for different needs per disciplines.
...
To mitigate further barriers (such as infrastructure, norms, incentives, policies, and funding), there is a need to liaise and coordinate actions with all stakeholders, both internal ones, like the Open Scholarship Support from Bodleian Libraries; the Research Data Management team in IT Services; the Ethics and Integrity unit in Research Service; but also external ones at the national level such as funders, & publishers.

While respondents thought overall that ORPs were beneficial, there were notable downsides mentioned by members of some fields, so there should no be a blanket policy. I think it would be useful to run consultations, notably in the humanities and social sciences to define what is beneficial to these fields and what would be detrimental. However, I think this need to occurs after researchers in those fields have had the opportunity to refine their discourse and consensus, which is one of the purpose of our current open humanities seminar series.

Finally, at the base of it all, there should be equitability of access to resources, infrastructure, and support, and this should be built in all programmes. 
