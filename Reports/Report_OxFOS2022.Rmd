...
![(\#fig:effect-double) Presumed overall effect of ORPs widespread adoption on the field of research of respondents; PGR students vs all other researchers combined (i.e. research staff and fellows, research support staff and academics).\
PGR students: MSD = `r sst_pgrdata$MSD[sst_pgrdata$Question == "Effect" ]`, MPLS = `r sst_pgrdata$MPLS[sst_pgrdata$Question == "Effect" ]`, SSD = `r sst_pgrdata$SSD[sst_pgrdata$Question == "Effect"]`, Hum = `r sst_pgrdata$Hum[sst_pgrdata$Question == "Effect"]`;\
Researchers: MSD = `r sst_allstaffdata$MSD[sst_allstaffdata$Question == "Effect" ]`, MPLS = `r sst_allstaffdata$MPLS[sst_allstaffdata$Question == "Effect" ]`, SSD = `r sst_allstaffdata$SSD[sst_allstaffdata$Question == "Effect"]`, Hum = `r sst_allstaffdata$Hum[sst_allstaffdata$Question == "Effect"]`](../Figures/Round12_Double_Effect-per-ORP.png)
...
Free text boxes opened for each ORP a respondent selected to have downsides. Their responses were roughly categorised, counted, and sorted in order of frequency in \@ref(tab:tblotherdownsides). <!--# this would need to be revised carefully to see if some categorisations were overwritten, whether some automatisation picked a word in a responses that should not be categorised this way, etc. Besides, last year, I tried to create large categories that could apply accross ORPs for a better read of the table summarising the counts of downsides for each ORP; I haven't revised my categories this year to do the same regrouping work so it's a bit all over the place...-->

Downsides to **Open Access** publications were the extreme cost of Gold OA, and the inequalities between researchers/fields/institutions that it creates depending on whether they have or do not have access to funding to publish with Gold OA. In addition, the mandates or moves to OA from funders and researchers, and the possibility for publishers to ask high APCs were believed to generate predatory behaviours from publishing companies, and to possibly lead to less invested editorial reviews (or less invested peer-review in general given the quantity or articles submitted and sent for review to generate income for publishers), and therefore, to lower the quality of research being published. Some respondents also thought they were less prestigious journal options that published OA<!--# more of a barrier, mentioned again there?-->. Finally, some respondents were concerned about the loss of income for scholarly society, authors, or publishers, if, for instance books or monographs, were available for free online.
...
Downsides to **Data sharing** were concerns over ethics, safety, or security (e.g. to preserve human participants anonymity, etc.) and the inability to control the validity of reuse, or the misrepresentation or misuse of the data shared. There were also relatively frequent concerns over intellectual property, fear of scooping, and general lack of recognition for the effort and cost of collecting primary data, especially if there were to be reused before the producing team was able to exploit their dataset fully in terms of article published. Less frequent concerns, but worth noting<!--# more of a barrier, mentioned again there?-->, respondents highlighted a lack of norm for dataset citation, as well as the lack of metadata standards to make data sharing usable and the fact that some data can be in proprietary format only, both of which making data sharing less useful or even irrelevant.
...
The main downsides to **Preprints** were the fact that they are disseminated and potentially used for policies or further research, while not being peer-reviewed. In lesser frequency but worth noting, downsides listed were fear of scooping, the race to publish first (possibly lowering the quality of the publication), the fact that it sometimes prevents formal publishing, concerns over the extreme growth of the literature on preprint servers, interference with double-blind peer-reviewing, and problem with updating citation (given duplicate versions) <!--# FIXME -->
...
Regarding **Preregistration**, some respondents thought it would impede exploratory research, impede flexibility in protocols while the research is conducted, and that this practice is not applicable to all fields of research (e.g. does not align with iterative development of research questions in e.g. social sciences) . In lesser frequency but worth noting, respondents mentioned that there need to be a corresponding increase in respect for null findings, and that if preregistration are made public before publication, it may deter unknowing replication.
...
For both **Preregistration** and **Registered Reports**, given this quotes and others (also in 'Barriers'), and the fact that awareness of these practices were overall very low and that we know that training is extremely limited around the University for these practices, I wonder, whether fields (particularly experimental fields) where preregistration was judged not applicable, or to have downsides such as 'impeding flexibility in protocol', do in fact operate under the same framework (Ho testing, alpha = 0.05) as fields who have judged that preregistration is necessary to prevent insight bias, confirmation bias, and insidious ways of p-hacking leading to drastic increases in the likelihood of obtaining being false-positive results. Do fields rejecting preregistration have the same ideal but simply don't have the right research environment (sometimes in very practical terms) to be able to practice preregistration, or is it that these fields approach are different and preregistration isn't relevant, e.g. if the research process resemble valid iterative processes as seen in some social sciences. I think there may be both type of responses in the survey. As a reminder, when one conducts an experiment, one state that they have an a priori hypothesis, and that a specifically designed manipulation will explain differences observed between groups (e.g. treatment vs control groups), I see no logical reasons (there may be very real practical barriers though) to not want to limit the flexibility in protocol or to not declare those deviation in order for readers to be able to assess the likelihood of each finding to be a false positive result (each decision of conducting a statistical test based on the p value of a first test will mathematically increase the chances of the second test to be a false positive finding, over the 5% tolerated and declared rate).

Perhaps further consultation, such as focus groups following an introduction to preregistration would be needed to know whether such responses constitute barriers over actual downsides . <!--# FIXME -->
...
Overall, despite these numerous downsides, remember that people seemed to think ORPs were largely favourable. So why aren't they more practised? Are they any barriers to adoptions?
...
Responses from students, research staff, and academics show overall similar patterns and therefore were merged (with research support staff which have very low sample sizes) in Figure \@ref(fig:barrier-single). <!--# this needs checking again at the end of data collection. mmmmhhhh not if split per Division, bur if merged Division, then true enough (final data 20220302). Plot split per Division is horrible to look at...-->
...
Barriers to Open Access publications were <!--# FIXME -->
...
Barriers to Data sharing were <!--# FIXME -->\
Some quotes: 1. "`r quote_OB_Data_1`"\
2. "`r quote_OB_Data_2`"

Barriers to Code sharing were <!--# FIXME -->\
Some quotes:\
1. "`r quote_OB_Code_1`"

Barriers to Material sharing were <!--# FIXME -->\
Some quotes:\
1. "`r quote_OB_Materials_1`"

Barriers to Preprints were <!--# FIXME -->\
Some quotes:\
1. "`r quote_OB_Preprint_1`"

Barriers to Preregistration were <!--# FIXME -->\
Some quotes:\
1. "`r quote_OB_Prereg_1`"

Barriers to Registered Reports were <!--# FIXME --> delays the start of data collection, which does not allow ECRs on short time contract to engage with the practice <!--# more of a barrier, mentioned again there?-->\
Some quotes:\
...
Besides these additions, the strongest barriers to adoptions of ORPs were incentives, training and norms.
...
-   How to prepare data and metadata for archiving, and possibly for sharing in a public repository in line with community standards [labelled **FAIR data**]
...
-   How to prepare materials for sharing (e.g. in a digital repository that assigns a DOI, or in a physical repository) [labelled **Open Meterial**]
...
Overall, the demand for training, especially workshop-led training (in red) is really high (see Figure \@ref(fig:training-single)).
...
For some training topics, the Humanities and the Social Sciences Division have less of a demand, which only correspond to a higher selection of the 'not applicable' option.
