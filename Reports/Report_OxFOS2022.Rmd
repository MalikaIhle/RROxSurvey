...
What Reproducible Research Oxford has been promoting so far are practices like Open Access publishing, Data, Code, and Material sharing, and use of Preprints, Preregistration, and Registered Reports. Definitions of relevant terms vary across fields of research and contexts, so here are the definitions we propose:
...
The purpose of the last two practices is to provide a transparent documentation, for instance, of the evolution of one's positionality  <!-- LF I don't know what this means... --> for qualitative research; but in the case of quantitative research, the main reason to engage with preregistration and/or registered reports  is to be able to distinguish between a priori confirmatory analyses, which test hypotheses and have a declared alpha of usually 5%, i.e. we tolerate to have a 5% chance for a statistical test to result in a false-positive finding, vs exploratory analyses that have a higher rate of false-positive results but have the advantages of generating hypotheses. <!-- LF not sure statistical details are necessary here... -->

<!-- FIXME add link to survey materials in Zenodo -->
...
The first round was aimed at post-graduate research (PGR) students based in any Divisions of the University and was administered between 12 January and 1 March 2021. The second round was administered between 12 January and 1 March 2022 and targeted all researchers in all Divisions and Colleges, i.e.\
- **Student on post-graduate research programme** (PGR, e.g. MSc by Research; MPhil; DPhil; EPSRC CDT; NERC DTP; BBSRC DTP; DClinPsych);\
...
Participation in the survey was voluntary and all responses were anonymous. This means that, while the survey was advertised through departmental newsletters (as well as several other channels of communication), people self-elected to participate; therefore, we can expect biases towards people already aware of these practices and/or those favourable to them. In addition, respondent anonymity also means that we cannot say whether some researchers (particularly PGR students) took part in both rounds of the survey.
...
Given the total number of researchers that had the opportunity to engage with the survey, and the proportion of respondents, it is unlikely that e.g. all students that responded in the second year also responded in the first year. In addition, PGR students patterns of responses in the second year looked similar to those in the first year. Therefore, the data was pooled across both years, and the cumulative sample size (max N) will be used.

Note that we have rather low absolute numbers for research support staff so while their responses will contribute to the total results, they will not be displayed in isolation as this could be misleading, especially when the results are also split per Divisions. <!-- LF or pool with research staff? -->

Table \@ref(tab:tabSSdiv) shows the number of responses in each of the Divisions (Humanities, abbreviated '**Hum**'; Mathematical, Physical, and Life Sciences, '**MPLS**'; Medical Sciences, '**MSD**'; Social Sciences, '**SSD**'; Department for Continuing Education, '**ContEd**'; Gardens, Libraries and Museums, '**GLAM**'), as well as responses from **College-only staff**. Note that College-only staff who also selected the Division  most closely aligned to their field of research were not included in the latter category; instead, they were added to the tally for the relevant Division.  <!-- LF is ContEd technically a Division? MI: we've had that discussion with DDR, it is a self-standing department... -->
...
We had the most responses from MPLS and MSD in terms of absolute numbers, then half of that for SSD and a third of that for Hum (but with still comparable level of percentages of the target population for the Humanities), <!-- LF wording unclear --> and then a few responses from GLAM, ContEd, and College-only staff who did not select a Division. All of the data will be used for overall results, but when showing plots with the data split by Division, only those relating to the four academic Divisions will be used.

In total, we had **`r N_div$BothRounds[N_div$Div == 'Total']`** participants in the survey. More precisely, this is the number of participants that, after giving their consent to participate in the survey, answered the two mandatory questions: Role and Affiliation. We kept respondents' answers whether they completed the survey or not. This means that the sample size slightly declines question after questions. The attrition rate (the percentage of participants dropping out of the survey after the first 2 mandatory questions and up to the last question) was `r sst_data$Total[sst_data$Question == 'PercDrop']`% on average, and increased from MSD, to MPLS, SSD, and Hum (see Table \@ref(tab:tabSSdivQ)). By the final question, we had **`r sst_data$Total[sst_data$Question == 'Support']`** remaining respondents (Table \@ref(tab:tabSSdivQ)). <!-- note that the last two lines of the table should be separated visually-->
...
For each question, I first looked at a 4-by-4 plot with all types of researchers (PGR students, research staff or fellows, research support staff, and academics) from each Division (MSD, MPLS, SSD, Hum) separated. From this, I decided to merge data with similar patterns for easier visualisation. In most questions, this resulted in splitting PGR students from other researchers (e.g. awareness/usage, training), but for some it resulted in splitting academics from non-academics (e.g. recruitment criteria, support needs).
...
Later on<!--# i.e. downsides question came after barriers questions; the rationale was to reduce amount of respondents listing barriers in downsides question -->, to understand why respondents thought a specific ORP was overall detrimental, and to include any downsides other respondents may find for a specific ORP, even if they judged it overall beneficial, we asked:
...
The main downside to **Open Access** publications frequently mentioned by respondents was the cost of Gold OA, and the inequalities between researchers/fields/institutions that it creates depending on access to funding. In addition, the mandates or moves to OA from funders and researchers, and the possibility for publishers to ask for high APCs were believed to generate predatory behaviours from publishing companies, and to possibly lead to less invested editorial reviews (or even less invested peer-review in general given the quantity of articles submitted and sent for review to generate income for publishers), and therefore, to possibly lower the quality of the research being published. Some respondents also thought there were fewer options of prestigious journals that published OA<!--# more of an infrastructure barrier, mentioned again there?-->. Finally, some respondents were concerned about the loss of income for scholarly society, authors, or publishers, if, for instance books or monographs, were available for free online.

Some quotes:\
1. "*`r quote_WD_OA_1`*" - source: `r quote_WD_OA_1_source`.\
2. "*`r quote_WD_OA_5`*" - source: `r quote_WD_OA_5_source`.\
3. "*`r quote_WD_OA_6`*" - source: `r quote_WD_OA_6_source`.\
4. "*`r str_replace(quote_WD_OA_7, "\n\n", " ")`*" <!--# shorten quote with "[...]"-->- source: `r quote_WD_OA_7_source`.\
5. "*`r quote_OB_OA_4`*" - source: `r quote_WD_OA_4_source`. <!--# cut?-->\
6. "*`r quote_WD_OA_8`*" - source: `r quote_WD_OA_8_source`.\

Downsides to **Data sharing** were mainly concerns over ethics, safety, or security (e.g. to preserve human participants anonymity) and the inability to control the validity of reuse, or the possible misrepresentation or misuse of the data shared. There were also relatively frequent concerns over intellectual property, fear of scooping, and general lack of recognition for the effort and cost of collecting primary data, especially if they were to be reused before the producing team was able to exploit their dataset fully in terms of article published. Less frequent concerns, but worth noting<!--# more of a lack of norm barrier, mentioned again there?-->, respondents highlighted a lack of norm for dataset citation, a lack of metadata standards to make data sharing usable, and the fact that some data can be in proprietary format only; the latter two making data sharing less useful or even irrelevant. 

Some quotes:\
1. "*`r quote_WD_Data_1`*" - source: `r quote_WD_Data_1_source`.\
2. "*`r quote_WD_Data_2`*" - source: `r quote_WD_Data_2_source`.\
3. "*`r quote_WD_Data_3`*" - source: `r quote_WD_Data_3_source`.\
4. "*`r quote_WD_Data_4`*" - source: `r quote_WD_Data_4_source`.\
5. "*`r quote_WD_Data_5`*" - source: `r quote_WD_Data_5_source`.\
6. "*`r quote_WD_Data_6`*" - source: `r quote_WD_Data_6_source`.\

Downsides to **Code sharing** were, similarly to data sharing, IP concerns, the fact that such  outputs may not be credited or recognised if reused by others, the absence of control over the validity of reuse, and ethical, safety, or security concerns. In addition, respondents mentioned that given that currently, research software is not reviewed, making it available for reuse could lower the quality of follow-up research. Less frequent, but worth noting, some respondents mentioned the psychological toll of having their code scrutinised and criticized by the community.

Some quotes:\
1. "*`r quote_WD_Code_1`*" - source: `r quote_WD_Code_1_source`.\
2. "*`r quote_WD_Code_2`*" - source: `r quote_WD_Code_2_source`.\
3. "*`r quote_WD_Code_3`*" - source: `r quote_WD_Code_3_source`.\
4. "*`r quote_WD_Code_4`*" - source: `r quote_WD_Code_4_source`.\

Downsides to **Material sharing** were ethical, safety, or security concerns (and possibly including laws against it<!--# more of a barrier, mentioned again there?-->), <!-- LF unclear what this relates to --> IP concerns, absence of control over the validity of reuse or misuse, scooping and lack of credit/recognition. In addition, respondents mentioned the potential huge cost of maintenance, and the unlikelihood <!-- FIXME not sure this is a word... --> of being able to provide equitable access to all.

Some original quotes highlighting some specific problems with physical material sharing:\
1. "*`r quote_WD_Materials_1`*" - source: `r quote_WD_Materials_1_source`.\
2. "*`r quote_WD_Materials_2`*" - source: `r quote_WD_Materials_2_source`.\
3. "*`r quote_WD_Materials_3`*" - source: `r quote_WD_Materials_3_source`.\
4. "*`r quote_WD_Materials_4`*" - source: `r quote_WD_Materials_4_source`.<!--# not sure this one is physical-->\

The main downside to **Preprints** was the fact that they are disseminated and potentially used for policies or further research before being peer-reviewed (if they ever get peer-reviewed) and therefore potentially containing errors or misleading readers. In lesser frequency but worth noting, other downsides listed were the fear of scooping, the race to publish first (possibly lowering the quality of the publication), the fact that it sometimes prevents formal publishing, concerns over the extreme growth of the literature on preprint servers, interference with double-blind peer-reviewing, and problem with updating citation in references manager software (given duplicate versions).

Some quotes:\
1. "*`r quote_WD_Preprint_1`*" - source: `r quote_WD_Preprint_1_source`.\
2. "*`r quote_WD_Preprint_2`*" - source: `r quote_WD_Preprint_2_source`.\
3. "*`r quote_WD_Preprint_3`*" - source: `r quote_WD_Preprint_3_source`.\
4. "*`r quote_WD_Preprint_4`*" - source: `r quote_WD_Preprint_4_source`.\
5. "*`r quote_WD_Preprint_5`*" - source: `r quote_WD_Preprint_5_source`.\
6. "*`r quote_WD_Preprint_6`*" - source: `r quote_WD_Preprint_6_source`.\
7. "*`r quote_WD_Preprint_7`*" - source: `r quote_WD_Preprint_7_source`.\
8. "*`r quote_WD_Preprint_8`*" - source: `r quote_WD_Preprint_8_source`.\

Regarding **Preregistration**, the main downsides described by respondents were that it was thought to impede exploratory research, impede flexibility in protocols while the research is being conducted, and that this practice is not applicable to all fields of research (for instance, it does not align with iterative development of research questions in e.g. social sciences). In lesser frequency but worth noting, respondents mentioned that there need to be a corresponding increase in respect for <!-- FIMXE LF: unclear --> null findings <!-- MI: how about this: (i.e. the purpose of preregistration is to limit the occurrence of false-positive finding, hence, preregistered research is possibly more likely than traditional research to lead to null-results. Null results aren't typically thought to be easy to publish in many journals. Therefore, if preregistraion are encouraged, null-result publication should be encouraged too) --> , and that if preregistrations are made public before publication, it may deter unknowing replication. <!-- FIMXE LF: unclear. MI: if preregistration are on open registreries prior to publication (e.g. for clinical research): before starting a study, people could check if this is being done, and give up on doing it if it is-->

Some quotes:\
1. "*`r quote_WD_Prereg_1`*" - source: `r quote_WD_Prereg_1_source`.\
2. "*`r quote_WD_Prereg_2`*" - source: `r quote_WD_Prereg_2_source`.\
3. "*`r quote_WD_Prereg_3`*" - source: `r quote_WD_Prereg_3_source`.\
4. "*`r quote_WD_Prereg_4`*" - source: `r quote_WD_Prereg_4_source`.\
5. "*`r quote_WD_Prereg_5`*" - source: `r quote_WD_Prereg_5_source`.\
6. "*`r quote_WD_Prereg_6`*" - source: `r quote_WD_Prereg_6_source`.\
7. "*`r quote_WD_Prereg_7`*" - source: `r quote_WD_Prereg_7_source`.\
8. "*`r quote_WD_Prereg_8`*" - source: `r quote_WD_Prereg_8_source`.\
...
Some quotes:\
1. "*`r quote_WD_RegRep_1`*" - source: `r quote_WD_RegRep_1_source`.\
2. "*`r quote_WD_RegRep_2`*" - source: `r quote_WD_RegRep_2_source`.\
3. "*`r quote_WD_RegRep_3`*" - source: `r quote_WD_RegRep_3_source`.\
4. "*`r quote_WD_RegRep_5`*" - source: `r quote_WD_RegRep_5_source`.\
5. "*`r quote_OB_Prereg_1`*" - source: `r quote_OB_Prereg_1_source`.\
6. "*`r quote_WD_RegRep_6`*" - source: `r quote_WD_RegRep_6_source`.\
7. "*`r quote_OB_RegRep_1`*" - source: `r quote_OB_RegRep_1_source`.\

For both **Preregistration** and **Registered Reports**, given these quotes and others (also in 'Barriers'), and the fact that awareness of these practices was overall very low and that we know that there is limited related training at the University, I wonder whether fields (particularly experimental fields) where preregistration was judged not applicable or unjustifiably limiting flexibility in protocols, do in fact operate under the same framework (Ho testing, alpha = 0.05) as fields who have judged that preregistration is necessary to prevent insight bias, confirmation bias, and non-intentional insidious ways of p-hacking leading to drastic increases in the likelihood of obtaining false-positive results. <!-- FIMXE unclear --> Do fields rejecting preregistration have the same ideal but simply don't have the right research environment (sometimes in very practical terms as seen in some quotes) to be able to practice preregistration, or is it that in these fields approaches are different and preregistration isn't relevant, e.g. if the research process resembles the iterative processes judged as a valid norm in some social sciences. I think there may be both types of responses in the survey. As a reminder, when one conducts an experiment, one states that they have an a priori hypothesis, and that a specifically designed manipulation will explain differences observed between groups (e.g. treatment vs control groups). Even after reading all these replies, I personally still see no logical reasons (there may be very real practical barriers though), for experimental fields to not want to limit the flexibility in protocol or to not declare those deviations in order for readers to be able to assess the likelihood of each finding to be a false positive result (another reminder: each decision of conducting a statistical test based on the p value of a first test will inevitably increase the chances of the second test to be a false positive finding, over the 5% tolerated and declared rate).<!--# FIXME language to not be offensive--> <!-- possibly rephrase to just say that it would be useful to offer more training to address common misconceptions (e.g. that you cannot do/report exploratory results), and then see if responses change -->

Perhaps further consultations, such as focus groups following an in depth introduction to preregistration would be needed to know whether the inability to create a plan a priori constitutes a barrier (e.g. practical impossibility) over an actual downside (e.g. the practice is irrelevant). <!--# FIXME language to not be offensive-->
...
Overall, despite these numerous downsides, remember that people seemed to think ORPs were largely favourable, <!-- LF do you mean "desirable" instead?--> at least for those that were known to them. So why aren't they more practised? Are there any barriers to the adoption of ORPs?
...
Although there were some differences between Divisions, overall responses from students, research staff, and academics showed roughly similar patterns and therefore were merged (with research support staff which have very low sample sizes) in Figure \@ref(fig:barrier-single). The plots shown below aren't stacked bar plots like in previous questions where respondents could only select one response; in this case, responses were not mutually exclusive. Given these bar plots are now dodged, <!-- FIXME wording --> I will not be showing differences between Divisions here, as it becomes a bit complicated to look at.

![(\#fig:barrier-single) Barriers to adoption of ORPs (non-mutually exclusive), with responses pooled across all Divisions, students and researchers.](../Figures/Round12_Single_AllDiv_Barriers.png)

<!-- FIXME chart titles are confusing --- all researchers (N = 1061) vs. PGRs and all reseachers (N = 1030)? -->

**Open Access** publishing's main barrier was considered to be the lack of incentives, and this was particularly so in the Humanities and Social Sciences Divisions across students and researchers (`r SSD_Hum_OA_Incentives`% for SSD and Hum on average, vs `r MSD_MPLS_OA_Incentives`% for MSD and MPLS on average; see Figure \@ref(fig:barrier-single-OA)). In the Social Sciences and Humanities Division, respondents were also more likely than in the other two Divisions to select additional barriers to adoption of OA publishing, particularly a lack of infrastructure (`r SSD_Hum_OA_Infrastructure`% for SSD and Hum on average, vs `r MSD_MPLS_OA_Infrastructure`% for MSD and MPLS on average; see Figure \@ref(fig:barrier-single-OA)).

![(\#fig:barrier-single-OA) Barriers to adoption of Open Access publishing (non-mutually exclusive), with responses pooled across all types of researcher and split by Division. MSD:`r Nalldata_Barriers_forOA_perDiv$N[Nalldata_Barriers_forOA_perDiv$Div == 'MSD']`, MPLS:`r Nalldata_Barriers_forOA_perDiv$N[Nalldata_Barriers_forOA_perDiv$Div == 'MPLS']`, SSD:`r Nalldata_Barriers_forOA_perDiv$N[Nalldata_Barriers_forOA_perDiv$Div == 'SSD']`, Hum:`r Nalldata_Barriers_forOA_perDiv$N[Nalldata_Barriers_forOA_perDiv$Div == 'Hum']`](../Figures/Round12_Single_splitDiv_Barriers_for_OA.png){width="400px"}
...
If a respondent selected that other barriers applied for a given ORP, a free text box was displayed for entering additional information. Responses were roughly categorised, counted, and sorted in order of frequency in Table \@ref(tab:tblotherbarriers). <!--# this would need to be revised carefully to see if some categorisations were overwritten, whether some automatisation picked a word in a responses that should not be categorised this way, etc. Besides, last year, I tried to create large categories that could apply across ORPs for a better read of the table summarising the counts of downsides for each ORP; I haven't revised my categories this year to do the same regrouping work so it's a bit all over the place...--> A selection of representative quotes, and of quotes that provide "unexpected" information, is also included below.
...
Some quotes:\
1. "*`r quote_OB_OA_1`*" - source: `r quote_OB_OA_1_source`.\
2. "*`r quote_OB_OA_2`*" - source: `r quote_OB_OA_2_source`.\
3. "*`r quote_OB_OA_3`*" - source: `r quote_OB_OA_3_source`.\ <!-- FIXME typo? -->

Barriers to **Data sharing** were ethical concerns, the lack of ownership of the data, the lack of support (including financial support) to prepare data for sharing, the lack of metadata standards, the fact that original cost of collecting primary data isn't recognised nor protected from scooping, <!-- FIXME unclear --- the cost or the data? -->  and the fact that repositories aren't user-friendly.

Some quotes:\
1. "*`r quote_OB_Data_1`*" - source: `r quote_OB_Data_1_source`.\
2. "*`r quote_OB_Data_2`*" - source: `r quote_OB_Data_2_source`.\

The main extra barrier to **Code sharing** was the time it takes to prepare such resource for sharing.

Some quotes:\
1. "*`r quote_OB_Code_1`*" - source: `r quote_OB_Code_1_source`.\
...
Some quotes:\
1. "*`r quote_OB_Materials_1`*" - source: `r quote_OB_Materials_1_source`.\
...
Some quotes:\
1. "*`r quote_OB_Preprint_1`*" - source: `r quote_OB_Preprint_1_source`.\

Barriers to **Preregistration** were the fact that it isn't adapted to all disciplines, the lack of funding, for instance to run pilot experiments before writing a preregistration, and the fear of scooping.

Some quotes:\
1. "*`r quote_OB_Prereg_1`*" - source: `r quote_OB_Prereg_1_source`.<!--# mentioned above -->\
2. "*`r quote_OB_Prereg_2`*" - source: `r quote_OB_Prereg_2_source`.\
...
Some quotes:\
1. "*`r quote_OB_RegRep_1`*" - source: `r quote_OB_RegRep_1_source`.<!--# mentioned above -->\
2. "*`r quote_OB_RegRep_2`*" - source: `r quote_OB_RegRep_2_source`\
...
These were additional barriers mentioned by some respondents. Overall, the main barriers to adoption of ORPs were the lack of training, norms, and incentives as seen in the previous section. We asked further questions to explore these barriers in more details.
...
All types of researcher across all Divisions agreed overall on the direction of change between currently used vs desirable recruitment criteria shown in Figure \@ref(fig:recruitment-combined). Namely, the importance of the number of publication, the prestige of outlet, and the amount or source of grant support should, according to respondents, drastically decrease, while the quality of publication should, according to them, be much more considerably taken into account when assessing researchers for recruitment at Oxford. Respondents also indicated that the importance of teaching, supervising and mentoring, and to some extent the use of ORPs, should also increase when assessing researchers for recruitment. <!--# FIXME numbers?-->

![(\#fig:recruitment-combined) Perceived current and desired future criteria for recruitment](../Figures/Round12_Single_AllDiv_Combined_Recruitment.png) <!-- FIXME one way to make this figure more compact/readable is to put y-axis labels between the two panels, as this looks like one plot anyway from the shared legend -->

Patterns or responses were similar between PGR students, research staff or fellows, and research support staff, and those patterns were slightly different from those shown by academics mostly for the fact the academics were overall less 'unsure' about currently used criteria. Given academics are the most likely researchers to have gone through a recruitment process for themselves and possibly to have been involved in the recruitment of others, it probably makes more sense to present results split between academics and others for these two questions (see Figures \@ref(fig:recruitment-current) and \@ref(fig:recruitment-future)).

There are lots of interesting results that I'm happy to provide to the DORA implementation team, but I don't have time to go into all of these details now. Very briefly, there are differences between Divisions regarding the desired importance of, among others, citation, impact, and teaching, the last two being an inverse mirror across Divisions (Figure \@ref(fig:recruitment-future)), etc. <!--# FIXME -->
...
nonacademic_ORP_considered_av <- round(summary(nonacademic_ORP_considered_perDiv$TotalPerc),1)
...
academic_ORP_considered_perDiv <-  academicdata_FutureRecruitment_for_plotting[
  ...
  academicdata_FutureRecruitment_for_plotting$Answer %in% c('Slightly', 'Moderately','Considerably'),] %>% 
    group_by(Div) %>% 
  summarise (TotalPerc = sum(perc))
academic_ORP_considered_av <- round(summary(academic_ORP_considered_perDiv$TotalPerc),1)
...
nonMSD_academic_ORP_notatall_perDiv <-  academicdata_FutureRecruitment_for_plotting[
  ...
  academicdata_FutureRecruitment_for_plotting$Answer %in% c('Not at all'),] %>% 
    group_by(Div) %>% 
  summarise (TotalPerc = sum(perc))
nonMSD_academic_ORP_notatall_av <- round(summary(nonMSD_academic_ORP_notatall_perDiv$TotalPerc),1)
...
Some respondents indicated additional recruitment criteria currently perceived to be important, rightfully or un-rightfully <!-- FIXME wording --> so, according to them; or conversely, un-rightfully missing recruitment criteria.

For instance, personal connections to people in the hiring department or prestige of former connections or institution were thought to be play a considerable role in current hiring decisions, but were often thought not to be desirable criteria.
...
Some quotes:\
1."*`r quote_Recruitement_1`*" - source: `r quote_Recruitement_1_source`.\
2."*`r quote_Recruitement_2`*" - source: `r quote_Recruitement_2_source`.\
...
Finally, addressing the other main barriers to adoption of ORPs, namely training and norms, we asked questions about training and community support needs.
...
training_needs <-  All_Grouped_Training_for_plotting[
    ...
    summarise (TotalPerc = sum(perc),1)
training_needs_av <- round(summary(training_needs$TotalPerc),1)
...
training_noneeds <-  All_Grouped_Training_for_plotting[
    ...
    summarise (TotalPerc = sum(perc))
training_noneeds_av <- round(summary(training_needs$TotalPerc),1)
...
Overall, the demand for guidance (written or workshop-led) was quite high, ranging from `r training_needs_av[1]`% to `r training_needs_av[6]`% ( `r training_needs_av[4]`% on average; see Figure \@ref(fig:training-single)) of respondents for all topics presented. The need for workshop-led training was lower for Open Access and Preprint than for other topics (workshop-led training: OA and Preprint: `r workshop_needs_OA_preprint[4]`% on average, other topics: `r workshop_needs_not_OA_preprint[4]`% on average) where guidance was simultaneously thought, a bit more frequently than other topics, to not be needed (no training needed: OA: `r training_noneeds_OA`%, Preprint: `r training_noneeds_preprint`%, other topics: `r training_noneeds_not_OA_preprint[4]`% on average). <!-- FIXME wording; rounding -->
...
For some training topics such as how to share code, or how to prepare preregistration, or how to prepare proposals for ethics review, some Divisions had less of a demand, with a corresponding increase in selection of the 'not applicable' option (Figure \@ref(fig:training-double-splitDiv)). <!--# FIXME more numbers ?-->
...
other_supports_essential_academics <- round(summary(All_academicdata_Support_for_plotting$perc[
                                All_academicdata_Support_for_plotting$LabelIndiv != 'Online Resources'&
                                All_academicdata_Support_for_plotting$Answer == 'Essential']),1)

other_supports_notuseful_academics <- round(summary(All_academicdata_Support_for_plotting$perc[
                                All_academicdata_Support_for_plotting$LabelIndiv != 'Online Resources'&
                                All_academicdata_Support_for_plotting$Answer == 'Not useful']),1)

other_supports_essential_nonacademics <- round(summary(All_alldata_but_academic_Support_for_plotting$perc[
                                All_alldata_but_academic_Support_for_plotting$LabelIndiv != 'Online Resources'&
                                All_alldata_but_academic_Support_for_plotting$Answer == 'Essential']),1)

other_supports_notuseful_nonacademics <- round(summary(All_alldata_but_academic_Support_for_plotting$perc[
                                All_alldata_but_academic_Support_for_plotting$LabelIndiv != 'Online Resources'&
                                All_alldata_but_academic_Support_for_plotting$Answer == 'Not useful']),1)
```
...
-   **Seminars** introducing relevant topics 

-   **Mentoring** from an expert in your field who has experienced similar challenges and can provide specialist advice as needed

-   **Coaching** support from a trained coach who can help you explore options and identify what may work best for you to tackle either focused problems or strategic longer-term issues

-   **Support networks** open to researchers from all career stages, with a focus on sharing experience and good practice

-   **Online resources** collating case studies and tips.

For this question, academics had slightly different patterns of responses than all the other types of researchers. While `r online_resources_essential_all[4]`% of academics and non-academics, on average, found online resources to be essential support, results differ for the other types of support proposed. Specifically, `r other_supports_essential_nonacademics[4]`% on average (range `r other_supports_essential_nonacademics[1]`% - `r other_supports_essential_nonacademics[6]`%) of PGR students and research staff found mentoring, support networks, coaching, and seminars essential, as opposed to `r other_supports_essential_academics[4]`% on average (range `r other_supports_essential_academics[1]`% - `r other_supports_essential_academics[6]`%) for academics. Accordingly, academics were more likely to find these other types of support to not be useful (`r other_supports_notuseful_academics[4]`% on average, vs `r other_supports_notuseful_nonacademics[4]`% for non academics). <!-- LF possibly because they feel they have no time for these... -->

![(\#fig:support-double-ac-vs-non-ac) Support needs for Academics and all other researchers and students](../Figures/Round12_Double_AllDiv_AcademicvsNonAcademic_Support.png)
...
Some respondents indicated additional support needs, which are listed in Table \@ref(tab:tblothersupport). The most frequently mentioned ones were a need for funding, the need for clear policies and University endorsement, and the need for advocacy work with stakeholders.
...
Our very last question related to open research that is *already* available at Oxford, with the aim to make an inventory of all initiatives at Oxford, so that we can promote them to relevant groups, and possibly come under the same umbrella to apply for institutional support.
...
Free text boxes were available for respondents to indicate any initiative they were aware of. Their responses were roughly categorised, counted, and sorted in order of frequency in Table \@ref(tab:tblinventory). There were very few initiatives we weren't already aware of.
...
Some respondents provided unsolicited general comments in this final question, some of which were quite contrasted, <!-- FIXME unclear --> which, at least, suggests that we captured the responses of some researchers who are *not* in favour of ORPs.<!--# in future iteration of the survey, would be best to have a final question asking for any remaining comment left... LF good idea! -->

Some quotes:\
1."*`r gsub("[^[:alnum:][:blank:]\\.]", "", Quote_Inventory_1)`*" - source `r Quote_Inventory_1_source`.\
2."*`r Quote_Inventory_2`*" - source `r Quote_Inventory_2_source`.\

### Summary

```{r, echo = FALSE, include = FALSE}

overall_unawareness <- round(summary(All_Grouped_Awareness_for_plotting$perc[All_Grouped_Awareness_for_plotting$Answer == 'Not aware / not sure if applicable']),1)
overall_practice <- round(summary(All_Grouped_Awareness_for_plotting$perc[All_Grouped_Awareness_for_plotting$Answer == 'Practicing myself']),1)

overall_aware_unpracticing <- All_Grouped_Awareness_for_plotting[
    All_Grouped_Awareness_for_plotting$Answer %in% c('Accessing / using only', 'Aware only'),] %>% 
    summarise (TotalPerc = sum(perc))
overall_aware_unpracticing_av <- round(summary(overall_aware_unpracticing$TotalPerc),1)

# I didn't make a double plot splitting students vs researchers *without* also splitting per Division so here the data to get overall stats for researchers only (just to add the few researchers who replied but aren't part of the 4 Div in the double plot presented in report:
Data_withoutstudents_Awareness <- rbind(staffdata_Awareness, supportstaffdata_Awareness, academicdata_Awareness)
Data_withoutstudents_Awareness_Split_Awareness_for_plotting <- prepare_data_for_plotting(Measures, Data_withoutstudents_Awareness, Awareness_answers, Awareness_columns)
Data_withoutstudents_Grouped_Awareness_for_plotting <- regroup_all_data(Data_withoutstudents_Awareness_Split_Awareness_for_plotting)

researchers_aware_unpracticing <- Data_withoutstudents_Grouped_Awareness_for_plotting[
    Data_withoutstudents_Grouped_Awareness_for_plotting$Answer %in% c('Accessing / using only', 'Aware only'),] %>% 
    summarise (TotalPerc = sum(perc))
researchers_aware_unpracticing_av <- round(summary(researchers_aware_unpracticing$TotalPerc),1)


overall_beneficial <- round(summary(All_Grouped_Effect_for_plotting$perc[All_Grouped_Effect_for_plotting$Answer == 'Beneficial']),1)
overall_no_downsides <- round(summary(All_Grouped_Downsides_for_plotting$perc[All_Grouped_Downsides_for_plotting$Answer == 'No']),1)

## barriers always tricky to work with - created new function that summarize data without splitting per Div
all_barriers_summary <- bind_summaries_barriers_items_without_Div_split(data_Barriers, Barriers_columns)
### not what we want... we want nb of rows with at least 1 responses that is a barrier (not NA, not sure, none) per ORP
# we could also want the average number of barriers respondent selected.
# created two functions that may need deleting

overall_training_need <- All_Grouped_Training_for_plotting[
    All_Grouped_Training_for_plotting$Answer %in% c("Written guidance and workshop-led training", "Written guidance only"),] %>% 
    summarise (TotalPerc = sum(perc))
overall_training_need_av <- round(summary(overall_training_need$TotalPerc),1)

overall_support_need <- All_Grouped_Support_for_plotting[
    All_Grouped_Support_for_plotting$Answer %in% c("Essential", "Useful"),] %>% 
    summarise (TotalPerc = sum(perc))
overall_support_need_av <- round(summary(overall_support_need$TotalPerc),1)

overall_barrier <- bind_summaries_barriers_items_without_Div_split(data_Barriers, Barriers_columns)
overall_atleastone_barrier <- round(summary(as.numeric(overall_barrier$perc_atleastone_barrier)),1)
overall_notsureNA_barrier <-round(summary(as.numeric(overall_barrier$perc_notsureNA_barrier)),1)
overall_nobarrier <-round(summary(as.numeric(overall_barrier$perc_nobarrier)),1)


```


Awareness and usage varied a lot between practices (`r overall_unawareness[4]`% unaware respondents on average, range: `r overall_unawareness[1]`% -`r overall_unawareness[6]`%; `r overall_practice[4]`% practicing respondents, range: `r overall_practice[1]`% - `r overall_practice[6]`% ) and to some extent varied by Division. A large proportion of respondents was aware of open research practices but did not practice them (`r overall_aware_unpracticing_av[4]`% of respondents on average, range: `r overall_aware_unpracticing_av[1]`% -`r overall_aware_unpracticing_av[6]`%), even when excluding students who may have had less opportunity to practice them (`r researchers_aware_unpracticing_av[4]`% of respondents on average, range: `r researchers_aware_unpracticing_av[1]`% -`r researchers_aware_unpracticing_av[6]`%). Is this due to downsides or barriers? 

The widespread adoption of ORPs was largely presumed to be beneficial (`r overall_beneficial[4]`% of respondents on average, range: `r overall_beneficial[1]`% -`r overall_beneficial[6]`%, variation matching the inverse variation in rates of 'unsure' replies), and the majority of respondents (`r overall_no_downsides[4]`% on average, `r overall_no_downsides[1]`% -`r overall_no_downsides[6]`%, variation also corresponding to rates of 'unsure' replies) thought there were no downsides associated with each ORP (with notable exceptions).

Around `r overall_atleastone_barrier[4]`% of respondents thought there was at least one barrier to adoption of ORPs, especially a lack of training, norms and incentives, followed by a lack of infrastructure, and policy. Another `r overall_notsureNA_barrier[4]`% specified they weren't sure or thought the question wasn't applicable to them. Finally, `r overall_nobarrier[4]`% on average thought there were no barriers to ORPs.

There was strong demand for training (online guidance and workshops, `r overall_training_need_av[4]`% on average, range: `r overall_training_need_av[1]`% - `r overall_training_need_av[6]`%), and in community support (e.g. mentorship, support networks; `r overall_support_need_av[4]`% on average, range: `r overall_support_need_av[1]`% - `r overall_support_need_av[6]`%).

There was general agreement on the direction of change between currently used vs desirable recruitment criteria that actually aligns with the DORA principles (no JIF based assessment); and there was variation in the importance of specific criteria between Divisions. 

### Recommendations

To raise awareness and adoption, the first recommendation would be to provide comprehensive and systematic training to all researchers, both staff and students, with a modular structure to account for different needs across disciplines and career stage.

Along with formal training, there should be a central place for online guidance where, ideally, researchers could go through the whole research cycle that’s relevant for their field and that sign-post everything they need to consider at each stage.

To help make the switch to open research practices normative, I think we should keep the community building exercises that many early career members of Reproducible Research Oxford lead and engage with, possibly also creating a more formal mentoring programme. I also think there should be some form of recognition associated with volunteering to give time to raise awareness and propagate norms to make such approach sustainable. 

In terms of bigger incentives, I think it would be very interesting and useful to run focus groups with academics to discuss possible change in recruitment criteria, especially with regards to ORPs and, considering the broader complex system, perhaps also considering team criteria (rather than just adding criteria on single individuals). <!-- FIXME I have reservations about this, so I'd be wary of recommending unless we have strong/further evidence -->

To mitigate further barriers (such as infrastructure, norms, incentives, policies, and funding), there is a need to liaise and coordinate actions with all stakeholders, both internal ones, like the Open Scholarship Support from Bodleian Libraries; the Research Data Management team in IT Services; the Ethics and Integrity unit in Research Services; but also external ones at the national level, such as funders and publishers.

While respondents thought overall that ORPs were beneficial, there were notable downsides mentioned by members of some fields, so there should not be a blanket policy and/or interventions. I think it would be useful to run consultations, notably in the humanities and social sciences, to define what is beneficial to these fields and what would be detrimental. However, I think this need to occurs after researchers in those fields have had the opportunity to refine their discourse and consensus, which is one of the aims of our current open humanities seminar series.

Finally, at the base of it all, there should be equity of access to resources, infrastructure, and support, and this should be built into all programmes. <!-- FIXME unclear what programmes refers to -->
